...
import uuid
from smart_problems_azureembeddings import sliding_window_chunk, get_embeddings_batched, build_inverted_index, hybrid_search, sentence_density_score, get_retrieval_queries, ask_gpt
import numpy as np
import re
import pdfplumber
import sys # to handle frozen state in PyInstaller
...
def get_resource_path(relative_path):
    """
    Get absolute path to resource, works for both dev and PyInstaller .exe
    """
...
def clean_old_session():
    print("Cleaning old session data...")
    ...
    # Clear live_transcript.txt
    try:
        with open(get_resource_path("live_transcript.txt"), "w") as f:
            f.write("")
    except Exception as e:
        print(f"Could not clear live_transcript.txt: {e}")

def start_monitor():
    script_name = 'monitor_transcription.py'

    # If running from a PyInstaller .exe, use the .exe instead of .py
    if getattr(sys, 'frozen', False):
...
@app.route("/vista_rpc", methods=["POST"])
def vista_rpc():
    data = request.get_json()
    station = data.get("station", "500")
    user_id = data.get("user_id", "983")
    context = data.get("context", "SDECRPC")
    rpc = data.get("rpc", "SDES GET USER PROFILE BY DUZ")
    parameters = data.get("parameters", [{"string": user_id}])
    try:
        token = get_jwt_token()
        result = call_rpc(token, station, user_id, context, rpc, parameters)
...
def process_chart_chunk():
    ...
    def load_prompt(field, fallback):
        try:
            with open(get_resource_path("prompts/explore/{field}.txt"), "r", encoding="utf-8") as f:
...
@app.route("/settings")
def settings():
    prompt_files = os.listdir(get_resource_path("templates/custom"))
...
def get_prompts():
    prompts = {}
    default_path = get_resource_path("templates/default")
    custom_path = get_resource_path("templates/custom")
...
def end_session():
    ...
    # Generate a filename with only the time in format '6-16-25@1130'
    now = datetime.now()
    filename = f"{now.month}-{now.day}-{str(now.year)[2:]}@{now.strftime('%H%M')}: .json"
...
def explore_search():
    ...
    from smart_problems_azureembeddings import get_retrieval_queries, hybrid_search, ask_gpt

    # Generate multiple focused queries from the user question
    queries = get_retrieval_queries(client, deploy_chat, query)
    print(f"[INFO] Retrieval queries: {queries}")

    # Run hybrid search for each query and combine results
    all_results = []
    for q in queries:
        all_results.extend(hybrid_search(client, deploy_embed, q, all_chunks, vectors, inverted_index, top_k=5))

    # Deduplicate by section/page/text
    seen = set()
    deduped_chunks = []
    ...
    # (Optional) Rerank: favor narrative (sentence-rich) chunks
    from smart_problems_azureembeddings import sentence_density_score
    deduped_chunks.sort(key=sentence_density_score, reverse=True)

    # Limit to top 20 chunks for GPT context
...
def select_patient():
    data = request.get_json()
    patient_dfn = data.get("patient_dfn")
    station = "500"
    provider_duz = "983"
    if not patient_dfn:
...
def get_patient():
    return jsonify(session.get("patient", {}))

@app.route("/lookup_patient", methods=["POST"])
def lookup_patient():
    data = request.get_json()
    search_type = data.get("type", "name")  # "name" or "icn"
    search_value = data.get("value")
    station = data.get("station", "500")
    user_id = data.get("user_id", "983")  # your DUZ
    if not search_value:
    ...
    try:
        ...
        if search_type == "icn":
            ...
            parameters = [{"string": search_value}]
        else:
            rpc = "ORWPT NAMELOOKUP"
            parameters = [{"string": search_value}]
        result = call_rpc(token, station, user_id, "SDECRPC", rpc, parameters)
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
