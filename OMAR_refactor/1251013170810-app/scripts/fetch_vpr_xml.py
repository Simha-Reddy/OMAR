...
def fetch_vpr_xml(
    *,
    ...
    Contract:
    - Inputs: DFN (patient_id), station, user_id, optional domain (mapped to TYPE), optional existing JWT token.
    ...
    Domain (TYPE) mapping (Vivian 8994-2917):
    ...
    If domain is provided we submit positional parameter sequence: DFN, TYPE.
    Otherwise we send only DFN (returns multi-domain <results> XML).
    """
    if not dfn:
    ...
    # Build positional parameter list: DFN [, TYPE]
    positional_params = [ {"string": str(dfn)} ]
    if domain:
        type_map = {
            'patient': 'demographics','med':'meds','lab':'labs','vital':'vitals','document':'documents',
            'image':'images','procedure':'procedures','visit':'visits','problem':'problems','allergy':'reactions'
        }
        type_val = type_map.get(str(domain))
        if type_val:
            positional_params.append({"string": type_val})
    # NOTE: start/stop/max/item/filter omitted for brevity; extend as needed.
