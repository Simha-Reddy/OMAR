...
def _archives_dir() -> Path:
    cfg = current_app.config.get('ARCHIVE_DIR')
    root = Path(cfg) if cfg else Path(__file__).resolve().parent.parent / 'archives'
    root.mkdir(parents=True, exist_ok=True)
    return root


def _archive_path(archive_id: str) -> Path:
    return _archives_dir() / f"{archive_id}.json"
...
def start_archive():
    ...
    with _archive_path(archive_id).open('w', encoding='utf-8') as f:
...
def list_archives():
    ...
    for path in _archives_dir().glob('*.json'):
        try:
            with path.open('r', encoding='utf-8') as f:
                doc = json.load(f)
