...
function parseChartPages(text) {
    ...
    if (matches[0] !== 0) matches.unshift(0); // Ensure first page starts at 0
...
window.runExploreSearch = async function () {
    ...
    document.getElementById("exploreGptAnswer").innerHTML = showThinkingSpinner();
    try {
        const res = await fetch("/explore_search", {
            ...
            body: JSON.stringify({
                query,
                qa_history: exploreQAHistory // <-- send history
            })
        });
        ...
        // Save Q&A to history
        exploreQAHistory.push({ question: query, answer: data.answer });
        // Render answer
...
async function fetchVistaProfile() {
    const res = await fetch("/get_patient");
    const patient = await res.json();
    if (!patient.user_id) {
        alert("No patient selected.");
        return;
    }
    const rpcRes = await fetch("/vista_rpc", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            station: patient.station,
            user_id: patient.user_id,
            // context, rpc, parameters as needed
        })
    });
    const data = await rpcRes.json();
    // Display or use data as needed
    console.log("VistA profile:", data);
}

async function updatePatientNameDisplay() {
    const res = await fetch("/get_patient");
    const data = await res.json();
    const el = document.getElementById("patientNameDisplay");
    if (el) el.textContent = data.name ? `Patient: ${data.name}` : "";
}
updatePatientNameDisplay();

document.getElementById("selectPatientBtn").onclick = async function() {
    const user_id = prompt("Enter patient DUZ or user ID:");
    if (!user_id) return;
    const station = "500";
    const res = await fetch("/select_patient", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id, station })
    });
    const data = await res.json();
    if (data.name) {
        alert(`Patient selected: ${data.name}`);
        updatePatientNameDisplay();
    } else {
...
document.getElementById("runRpcBtn").onclick = async function() {
    const rpcName = document.getElementById("rpcNameInput").value.trim();
    const paramsRaw = document.getElementById("rpcParamsInput").value.trim();
    const resultBox = document.getElementById("rpcResultBox");

    // Get patient from session (for station/user_id)
    const patientRes = await fetch("/get_patient");
    const patient = await patientRes.json();
    const station = patient.station || "500";
    const user_id = patient.user_id || "983";

    if (!rpcName) {
        resultBox.textContent = "Please enter an RPC name.";
        return;
    }
    let parameters = [];
    if (paramsRaw) {
        try {
            ...
            if (!Array.isArray(parameters)) throw new Error("Parameters must be a JSON array.");
        } catch (e) {
            resultBox.textContent = "Invalid parameters: " + e.message;
            return;
        }
    } else {
        // Default: use current patient user_id if available
        if (patient.user_id) {
            parameters = [{ "string": patient.user_id }];
        }
    }

    resultBox.textContent = "Running RPC...";
    try {
        const res = await fetch("/vista_rpc", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                station,
...
document.getElementById("lookupPatientBtn").onclick = async function() {
    const value = document.getElementById("patientSearchInput").value.trim();
    const type = document.getElementById("patientSearchType").value;
    ...
    try {
        const res = await fetch("/lookup_patient", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        // For name lookup, show a list and let user pick
        if (Array.isArray(data)) {
            resultsDiv.innerHTML = data.map(
                (p, i) => `<button onclick="selectPatientDFN('${p.dfn}', '${p.name}')">${p.name} (DFN: ${p.dfn})</button>`
            ).join("<br>");
        } else if (data.dfn) {
            selectPatientDFN(data.dfn, data.name || data.dfn);
        } else {
