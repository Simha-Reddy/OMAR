...
const SessionManager = {
  ...
  async saveToSession() {
    try {
      // Check gating condition: start only when transcript or visit notes has content
      ...
      const shouldStart = hasTranscript || hasVisitNotes;
  ...
  async clearSession() {
    try {
      ...
      if (response.ok) {
        ...
        window.exploreQAHistory = []; // <-- Add this line
      } else {
  ...
  async endSession(sessionName) {
    ...
    try {
      ...
      await fetch('/scribe/clear_live_transcript', { method: 'POST' });
  ...
  collectData: async function() {
    ...
    // Scribe page data
    // Fetch from backend, fall back to text box if fetch fails
    let transcript = '';
    try {
        const res = await fetch('/scribe/live_transcript');
  ...
  async restoreData(data) {
    ...
    if (data.scribe) {
      ...
      // If restored content includes transcript or visit notes, enable autosave
      try {
        ...
        if (hasT || hasV) this.autosaveStarted = true;
      } catch(_e) {}
...
if (document.getElementById('rawTranscript') || document.getElementById('chunkText')) {
  window.addEventListener('beforeunload', async () => {
    try {
      ...
      const hasTranscript = !!(transcriptEl && transcriptEl.value && transcriptEl.value.trim().length);
      const hasVisitNotes = !!(visitNotesEl && visitNotesEl.value && visitNotesEl.value.trim().length);
      if (!(hasTranscript || hasVisitNotes)) return; // nothing to archive yet
    } catch(_e) {}
